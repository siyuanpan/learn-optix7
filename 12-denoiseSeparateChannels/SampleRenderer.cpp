...
SampleRenderer::SampleRenderer(const Model *model, const QuadLight &light)
  ...
  std::cout << "setting up cuda module ..." << std::endl;
  createCudaModule();
...
void SampleRenderer::createCudaModule() {
  cuda_module.init("toneMap",
                   "C:/Users/pansiyuan/dev/learn-optix/"
                   "12-denoiseSeparateChannels/toneMap.cu",
                   compute_capability, MAX_REGISTERS);

  kernel_tone_map.init(&cuda_module, "computeFinalPixelColorsKernel");
}
...
void SampleRenderer::downloadPixels(uint32_t h_pixels[]) {
  finalColorBuffer.download(
      h_pixels, launchParams.frame.size.x * launchParams.frame.size.y);
}

inline vec2i divRoundUp(vec2i a, vec2i b) { return (a + b - vec2i(1)) / b; }
...
void SampleRenderer::computeFinalPixelColors() {
  ...
  kernel_tone_map.set_grid_dim(numBlocks.x, numBlocks.y, 1);
  kernel_tone_map.set_block_dim(blockSize.x, blockSize.y, 1);
  kernel_tone_map.execute((uint32_t *)finalColorBuffer.d_pointer(),
