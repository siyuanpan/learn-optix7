...
struct SampleWindow : public GLFCameraWindow {
  SampleWindow(const std::string& title, const Model* model,
               const Camera& camera, const QuadLight& light,
  ...
  virtual void key(int key, int mods) {
    if (key == 'D' || key == ' ' || key == 'd') {
      ...
      std::cout << "denoising now " << (sample.denoiserOn ? "ON" : "OFF")
                << std::endl;
    }
    if (key == 'A' || key == 'a') {
      sample.accumulate = !sample.accumulate;
      std::cout << "accumulation/progressive refinement now "
                << (sample.accumulate ? "ON" : "OFF") << std::endl;
    }
    if (key == ',') {
...
int main(int ac, char** av) {
  try {
    ...
    SampleWindow* window = new SampleWindow("Optix 7 Course Example", model,
                                            camera, light, worldScale);
